...
{{include "rules/R9/canonical-example.md"}}
...
- Discovery order: `.ai/` → `.ainav/` → `docs/`. Use the first that exists; create
  `docs/` if none does.
- Monorepo: each sub-project (its own `{{.ProjectMarker}}` or equivalent sub-project boundary)
  gets its own doc root and index; the repo-root index links the sub-indexes.
  Mechanical discovery is keyed on `{{.ProjectMarker}}` — a sub-project in another language
...
{{include "rules/R9/falsifying-questions.md"}}
