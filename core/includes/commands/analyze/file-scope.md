...
1. **An argument names it** (`$ARGUMENTS`). A file pattern (`./pkg/parser/*{{.SrcExt}}`,
...
2. **The working tree.** Files changed against `HEAD`, staged, unstaged and untracked,
   ...
   ```bash
   { git diff --name-only --diff-filter=ACMR HEAD; git ls-files --others --exclude-standard; } | grep '\{{.SrcExt}}$' | sort -u
   ```
3. **The current PR.** With a clean tree, the files changed on this branch since its base
   ...
   ```bash
   git diff --name-only --diff-filter=ACMR "$(git merge-base HEAD origin/main 2>/dev/null || git merge-base HEAD main 2>/dev/null || git merge-base HEAD master)"..HEAD | grep '\{{.SrcExt}}$'
   ```
