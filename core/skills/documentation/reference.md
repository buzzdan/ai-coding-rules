...
{{include "skills/documentation/reference/menus-contents-line.md"}}
...
{{include "skills/documentation/reference/decoder-ring-example.md"}}
...
{{include "skills/documentation/reference/doc-comment-menus.md"}}
...
{{include "skills/documentation/reference/conventions-scope.md"}}
```
...
- Monorepo: each sub-project (own `{{.ProjectMarker}}` or equivalent boundary) gets its own doc
...
2. **The package {{.DocForm}}** — when the doc spans a whole package rather than one
...
`{{.CommentPrefix}} See <docroot>/<file>.md for <three-to-six-word reason>.` Never restructure the
...
- [ ] Exported symbols carry WHY — rationale, incident, constraint — never a restated
      identifier
{{include "skills/documentation/reference/comment-checklist-tail.md"}}
...
{{include "skills/documentation/reference/bug-fix-examples.md"}}
...
{{include "skills/documentation/reference/runnable-example.md"}}
