...
allowed-tools:
  ...
  - Grep
  - Bash
  - Skill({{.Plugin}}:linter-driven-development)
---
...
repository that has never run {{.CmdPrefix}} is a different job: expect the loop to run
long, and prefer `/{{.CmdPrefix}}-analyze --all` first to see the size of it.
...
1. **An argument names it** (`$ARGUMENTS`). A file pattern or a directory fixes those
   ...
   repository, fixes every `{{.SrcGlob}}` file. The whole repository is never touched
   without being asked.
...
**Use the Skill tool** to invoke `Skill({{.Plugin}}:linter-driven-development)` with the
resolved scope, telling it Phases 1–2 are skipped:

**Phase 3 — TESTS + LINT over the scope** (via the `{{.Plugin}}:lint-fixer` agent, Agent tool,
isolated context, spawned with the package list)
- Discover project test/lint commands (`task test` / `make test` / `{{.DefaultTest}}`;
  lint from Taskfile/Makefile or `{{.DefaultLintFix}}`)
...
- @pre-commit-review orchestrates parallel `{{.Plugin}}:rule-hunter` agents + the
  `{{.Plugin}}:overabstraction-skeptic` against the diff; it reports, never edits, never widens
