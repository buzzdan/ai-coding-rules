...
- **Tests failing?** → Fix tests, then run `/{{.CmdPrefix}}-analyze`
- **Linter errors?** → Run `/{{.CmdPrefix}}-quickfix` for auto-fix loop
- **Code complete?** → Run `/{{.CmdPrefix}}-review` for commit readiness check
- **Starting new work?** → Run `/{{.CmdPrefix}}-autopilot` for full workflow
