...
2. **Linter (report-only)**: `Bash([discovered lint command, no --fix])` — surfaces
   what needs refactoring without changing anything. (The `{{.Plugin}}:lint-fixer` agent, which
...
3. **Design review**: invoke `Skill({{.Plugin}}:pre-commit-review)`
   ...
   spawns one parallel `{{.Plugin}}:rule-hunter` per rule family with hits (at most four), runs the
   `{{.Plugin}}:overabstraction-skeptic` over every type/package-extraction proposal, and returns
...
- ✅/⚠️ **Review**: clean / findings, categorized as the pre-commit-review report returns them:
  - 🐛 **Bugs** — fail at runtime regardless of rule (incl. R10 {{.Task}} leaks and unguarded concurrent writes)
...
/{{.CmdPrefix}}-analyze ./pkg/parser/parser{{.SrcExt}}

# Audit the whole repository — only ever on request
/{{.CmdPrefix}}-analyze --all
```
