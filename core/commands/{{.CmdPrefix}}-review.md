...
> For the tests and the linter beside the review, use `/{{.CmdPrefix}}-analyze`; for
> auto-fix capability, use `/{{.CmdPrefix}}-quickfix`.
...
1. **An argument names it.** A file pattern reviews those files. `--all` — or an explicit
   request to audit the whole repository — reviews every `{{.SrcGlob}}` file. The whole
...
1. **No build, no tests, no linters.** A review-only command runs none of them: the
   ...
   or lint commands; `/{{.CmdPrefix}}-analyze` is the command that runs those gates
   beside the review.
2. **Review**: invoke `Skill({{.Plugin}}:pre-commit-review)` in FULL mode over the resolved
   scope, passing it the file list. It orchestrates parallel `{{.Plugin}}:rule-hunter` agents
   (one per rule family with hits, four at most) + the `{{.Plugin}}:overabstraction-skeptic`
...
Use when you want the design review on the code you are about to commit, without changes and without waiting for the tests and the linter. This is the Phase 4 review on its own; `/{{.CmdPrefix}}-analyze` adds the tests and lint gates, `/{{.CmdPrefix}}-quickfix` the auto-fix loop.
