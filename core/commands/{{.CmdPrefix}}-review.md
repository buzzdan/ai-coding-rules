...
2. **Run in read-only mode**:
   ...
   - Review: invoke `Skill({{.Plugin}}:pre-commit-review)` in FULL mode. It orchestrates parallel `rule-hunter` agents + the `overabstraction-skeptic` and reports — it never edits.
