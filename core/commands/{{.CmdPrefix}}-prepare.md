...
4. **Apply survivors** via `Skill({{.Plugin}}:refactoring)` in
...
gates, use `/{{.CmdPrefix}}-quickfix`; for a read-only assessment, `/{{.CmdPrefix}}-analyze`.
