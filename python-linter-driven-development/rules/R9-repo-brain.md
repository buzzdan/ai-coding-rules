...
drift — the code *is* the behavior. Rung 1 drifts slowly: a docstring lives
...
symbol → its docstring → the feature doc; CLAUDE.md → index.md → the feature doc. An
...
| 1 | Code comments (docstring) | low — lives beside the code, reviewed with diffs | the WHY within a tiered 1–5 prose-line budget (policy below); network edges: `See docs/<feature>.md` |
...
cap math, the docstring states the incident — each level adds detail. Bad overlap: the
...
- **Guarantees** — thread safety, None handling, invariants
...
**Visibility default — underscore-prefixed symbols get no comment.** The tier table
prices exported API. An underscore-prefixed function, type, constant, or variable
...
- **Package docs in a dedicated file**: a package that genuinely earns more (data-flow
  ...
  package docstring to the language's dedicated package-doc file, bounded at ~20–30 lines. A package comment
...
- **Crossroads expand recommendation**: the writer never self-exceeds the 5-line
  cap. When a critical crossroads would benefit from richer inline docstring beyond the
  doc reference, write within budget and append an optional, end-of-report
  recommendation — `consider expanding <Symbol>'s docstring inline — <rationale>` —
...
- Discovery order: `.ai/` → `.ainav/` → `docs/`. Use the first that exists; create
  `docs/` if none does.
- Monorepo: each sub-project (its own `pyproject.toml` or equivalent sub-project boundary)
  gets its own doc root and index; the repo-root index links the sub-indexes.
  Mechanical discovery is keyed on `pyproject.toml` — a sub-project in another language
  ...
  adapter block; this build carries the Python adapter.
