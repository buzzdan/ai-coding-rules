...
- **Tests failing?** → Fix tests, then run `/py-ldd-analyze`
- **Linter errors?** → Run `/py-ldd-quickfix` for auto-fix loop
- **Code complete?** → Run `/py-ldd-review` for commit readiness check
- **Starting new work?** → Run `/py-ldd-autopilot` for full workflow
