...
name: comment-critic
description: |
  WHEN: Spawned programmatically by the documentation skill (after it writes docstrings
...
comments. Directives (`# noqa`, `# type: ignore`, `# pragma: no cover`, `# fmt: off`/`# fmt: on`, `# pylint:`, `# ruff:`, `# isort:`, and `>>>` doctest lines with their expected output) are not comments; skip
them.
...
an underscore-prefixed function, type, constant, or variable, the default verdict is
...
```
file:line | kind (docstring/in-body/test) | KEEP / TRIM / REWRITE / DELETE (/ DELETE → route R3)
