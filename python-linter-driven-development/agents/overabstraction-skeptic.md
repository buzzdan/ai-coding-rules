...
constructor, underscore-prefixed fields, an options mechanism with its `With*` functions, a
named Null Object default — these carry no juiciness of their own and are never
scored: they are how R2 closes the default-value and None holes of a type that
...
is no argument against them (one production caller is the normal case), and a None-guard
R2 deletes from a method is never a "regression": the default-constructed value is R2's
hole, closed by underscore-prefixed fields behind the constructor, not by the guard. Judge the *type* a finding
...
2. Score the proposed extraction against every block of the pasted scorecard, the
   ...
   validating constructor as the only public entry (underscore-prefixed fields, R2); and the two
