...
name: code-designing
description: |
  FORWARD view over rules/ — domain type design and architectural planning for Python code BEFORE it exists.
...
Scan the codebase structure: vertical (`app/feature/{handler,service}.py`, one
package per feature) vs horizontal (`app/{handlers,services}/feature.py`, or one
`models.py`/`views.py`/`services.py` per layer)?
...
| `../../rules/R2-self-validating-types.md` | Constructor-only entry, validation ownership, trusting composed values, None is not a value, no defensive checks in methods. |
...
Package Structure:
[feature]/
  ├── __init__.py      # the public surface: what a consumer imports
  ├── [type].py        # each juicy type in its own module
  ├── service.py
  └── handler.py
