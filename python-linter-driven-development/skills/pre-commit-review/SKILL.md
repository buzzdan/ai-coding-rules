...
`python-linter-driven-development:comment-critic`, spawned by those names and no others. Pure orchestration
...
- **Diff scope**: the caller's resolved file list or diff range — an explicit argument,
  ...
  '*.py'` plus untracked files), else the branch against its base, and the whole
...
- `diff.patch` — on a scoped review, `git diff <range> -- '*.py'`; untracked
