...
parallel single-obsession `python-linter-driven-development:rule-hunter` agents, one `python-linter-driven-development:overabstraction-skeptic`, and one
`python-linter-driven-development:comment-critic`.
...
- **Diff scope**: the caller's resolved file list or diff range. Callers resolve it by one
  ...
  (`git diff --name-only HEAD -- '*.py'` plus untracked files), else the current
...
assertion library), a new dependency in `pyproject.toml`, a new tool or config file, edits to
...
- For a scoped review: `diff.patch`, the diff over the scope (`git diff <range> --
  '*.py'`, untracked files appended whole), and `scope.txt`, the full text of
...
For every rule with pre-filter hits, spawn one `python-linter-driven-development:rule-hunter` agent, as **foreground**
...
"Strategy Map" proposals — and spawn one `python-linter-driven-development:overabstraction-skeptic`, as a **foreground**
...
construction mechanics — a validating constructor, underscore-prefixed fields, an options
...
(any hit qualifies; directives don't count) — spawn one `python-linter-driven-development:comment-critic` in the same
...
3. The absolute path to `../../examples/private-comment-noise.md` — the critic
   reads it when judging comments on underscore-prefixed symbols.
4. The diff scope and the scope bundle's absolute path.

It judges every comment in the diff (docstring, in-body, test) against the three-test
...
fields, a method that re-checks them and a None handed to the constructor are three
...
🔗 CLUSTER: Alert.Channel
   ...
   unknown-channel default at notify.py:71 goes; +2 noun — Send/Deliver)
...
- 🐛 **Bugs** — will fail at runtime regardless of rule (None returned as a value,
...
```
📊 CODE REVIEW REPORT
Scope: user/service.py, user/auth.py (+ tests) · Mode: FULL
...
- Spawn anything other than `python-linter-driven-development:rule-hunter`, `python-linter-driven-development:overabstraction-skeptic`, and
  `python-linter-driven-development:comment-critic`
