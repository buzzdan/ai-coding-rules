...
narration in a docstring is provenance, not behavior (R9's floor names it a failure
mode).
...
test; applies to docstrings and feature docs alike).
...
3. **Rung 1 — docstring**: write/refresh doc comments per R9's comment policy —
   **exported symbols only by default**: an underscore-prefixed symbol gets NO comment
   ...
   earns more moves its docstring to the language's dedicated package-doc file
   (R9's ~20–30 line bound). A crossroads that deserves richer inline docstring
...
- docstring: <symbols touched, grouped by package>
...
- <Symbol> — consider expanding its docstring inline beyond the doc reference:
  <one-line rationale>
```
...
- Spawn anything other than `python-linter-driven-development:comment-critic`, loop the critique more than one
