...
Thread safety, None handling, invariants — promises the signature cannot express.
...
underscore-prefixed symbol defaults to **zero** comment lines; its name is the
...
Code↔docs checks cover Python files; docs about other languages get the structure
...
- Monorepo: each sub-project (own `pyproject.toml` or equivalent boundary) gets its own doc
...
2. **The package docstring** — when the doc spans a whole package rather than one
...
- UserService assumes repository is never None (validated in constructor)
```
