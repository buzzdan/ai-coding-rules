...
description: |
  META ORCHESTRATOR for any code change that should end in a commit (features, bug fixes, refactors), when no language-specific linter-driven-development plugin covers the repository's language.
  WHEN: User requests code work (implement, fix, add, refactor), mentions "@ldd"/"ldd", or runs a /ldd-* command.
...
allowed-tools:
  ...
  - Skill(linter-driven-development:refactoring)
  - Skill(linter-driven-development:pre-commit-review)
  - Skill(linter-driven-development:documentation)
  - Agent
---

<objective>
Top-level protocol for any-language implementation work: five phases plus the autonomous
...
`linter-driven-development:lint-fixer` agent, at the cadence each check's economics demand.
</objective>

<triggers>
- User requests any-language code work (implement, fix, add, refactor, update, change) and the
  project is any-language (`the language's project manifest` or `.<ext>` files present)
- User mentions "ldd" or "@ldd"
- A `/ldd-*` command invokes this skill
On trigger, announce: **"Using ldd workflow for this any-language code work"** and run pre-flight.
</triggers>
...
| @refactoring | `Skill(linter-driven-development:refactoring)` |
...
`subagent_type: "linter-driven-development:lint-fixer"`. That is the Agent tool's one use in this
...
4. **SKEPTICIZED** — any prep move that creates a type/interface/package is judged by
   the `linter-driven-development:overabstraction-skeptic` (Agent tool; spawn prompt per @pre-commit-review step 3), with
...
1. Package-scoped lint (fast): the discovered lint command in report mode, narrowed
   to the package or directory just written — never the whole repository here.
...
<phase_3_full_lint>
Delegate ONE lint run to the `linter-driven-development:lint-fixer` agent (Agent tool, isolated context — the
...
the entry was a scoped command (`/ldd-quickfix` names the rung and the files).
...
<phase_5_ship>
1. Invoke @documentation (FEATURE mode): doc comment + feature docs, wired into the
