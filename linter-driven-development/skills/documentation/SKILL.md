...
narration in a doc comment is provenance, not behavior (R9's floor names it a failure
mode).
...
test; applies to doc comments and feature docs alike).
...
3. **Rung 1 — doc comment**: write/refresh doc comments per R9's comment policy —
   **exported symbols only by default**: an internal symbol gets NO comment
   ...
   earns more moves its doc comment to the language's dedicated package-doc file
   (R9's ~20–30 line bound). A crossroads that deserves richer inline doc comment
...
7. **Comment critique**: spawn the `linter-driven-development:comment-critic` agent (Agent tool) on the full diff —
   ...
   `../../rules/R9-repo-brain.md`, naming its comment-policy section (toolbox kinds,
   three-test standard, tiers, budget accounting, visibility default); (b) the
   absolute path of this skill's `reference.md`, naming its Comment Value Toolbox
   catalog — the critic reads both in its first turn; nothing is pasted, and this
   skill reads neither to spawn it; (c) the absolute path to
...
6. **Wire missing upward edges**: for each indexed (non-stale) doc with no code-side
   edge, add ONE line — `<comment-marker> See <docroot>/<file>.md ...` — to the front-door anchor's
   existing doc comment (anchor heuristic in reference.md), then confirm the file
   ...
   of improvising. Wiring only: never rewrite the comment around it, never wire a stale
...
- doc comment: <symbols touched, grouped by package>
...
- <Symbol> — consider expanding its doc comment inline beyond the doc reference:
  <one-line rationale>
```
...
- Spawn anything other than `linter-driven-development:comment-critic`, loop the critique more than one
