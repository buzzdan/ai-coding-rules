...
- **Diff scope**: the caller's resolved file list or diff range — an explicit argument,
  ...
  'detected-language source'` plus untracked files), else the branch against its base, and the whole
...
repo does not already use (a new test mechanism, a dependency in `the language's project manifest`,
...
- `diff.patch` — on a scoped review, `git diff <range> -- 'detected-language source'`; untracked
