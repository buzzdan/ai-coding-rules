...
name: code-designing
description: |
  FORWARD view over rules/ — domain type design and architectural planning for code BEFORE it exists, in whatever language the repository uses.
...
Scan the codebase structure: vertical (`<feature>/{handler,service}.<ext>`) vs
horizontal (`{handlers,services}/<feature>.<ext>`)?
...
Package Structure:
[feature]/
  ├── [type].<ext>      # each juicy type in its own file
  ├── service.<ext>
  └── handler.<ext>
