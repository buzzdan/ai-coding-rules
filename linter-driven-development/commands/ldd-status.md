...
- **Tests failing?** → Fix tests, then run `/ldd-analyze`
- **Linter errors?** → Run `/ldd-quickfix` for auto-fix loop
- **Code complete?** → Run `/ldd-review` for commit readiness check
- **Starting new work?** → Run `/ldd-autopilot` for full workflow
