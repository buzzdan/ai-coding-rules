...
Invoke `Skill(linter-driven-development:documentation)` and run its **BOOTSTRAP
...
6. **Upward edges wired**: every confidently-anchorable doc gets its one-line
   `<comment-marker> See <docroot>/<file>.md ...` edge on its front-door symbol
...
- Touch anything beyond doc files, `index.md`, `conventions.md`,
  CLAUDE.md/AGENTS.md, the copied check script, and one-line doc comment edge additions
