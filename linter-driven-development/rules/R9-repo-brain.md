...
drift — the code *is* the behavior. Rung 1 drifts slowly: a doc comment lives
...
symbol → its doc comment → the feature doc; CLAUDE.md → index.md → the feature doc. An
...
The retry loop lives in retry/policy.<ext> around line 40; it uses full jitter.
```
...
```text
<comment-marker> Policy is a capped exponential-backoff retry policy with full jitter.
<comment-marker> Jitter is deliberate: synchronized clients retrying in lockstep re-overloaded
<comment-marker> the upstream API after every blip. See docs/retry-policy.md for the incident
<comment-marker> and the cap math.
Policy
    maxAttempts, baseDelay
...
names `attempts`/`backOff` carry it (`R3-storifying.md`); `Policy`'s doc comment
...
| 1 | Code comments (doc comment) | low — lives beside the code, reviewed with diffs | the WHY within a tiered 1–5 prose-line budget (policy below); network edges: `See docs/<feature>.md` |
...
cap math, the doc comment states the incident — each level adds detail. Bad overlap: the
...
- **Guarantees** — thread safety, null handling, invariants
...
- **Package docs in a dedicated file**: a package that genuinely earns more (data-flow
  ...
  package doc comment to the language's dedicated package-doc file, bounded at ~20–30 lines. A package comment
...
- **Crossroads expand recommendation**: the writer never self-exceeds the 5-line
  cap. When a critical crossroads would benefit from richer inline doc comment beyond the
  doc reference, write within budget and append an optional, end-of-report
  recommendation — `consider expanding <Symbol>'s doc comment inline — <rationale>` —
...
- Discovery order: `.ai/` → `.ainav/` → `docs/`. Use the first that exists; create
  `docs/` if none does.
- Monorepo: each sub-project (its own `the language's project manifest` or equivalent sub-project boundary)
  gets its own doc root and index; the repo-root index links the sub-indexes.
  Mechanical discovery is keyed on `the language's project manifest` — a sub-project in another language
  ...
  adapter block; this build carries the any-language adapter.
...
one command answers all four. Code-side searches run over the language's source
...
2. **Is any edge broken — in either direction?**
   ...
   Detection, docs→code: build the repo's declaration set once — every declared
   type, function, method, constant and module or package name, in the shapes the
   language declares them — and resolve each backticked symbol against it. A token
   missing from the set still resolves when it appears as a whole word in any
   ...
   ban: URL spans (package-documentation links, say), fenced code blocks, and glob
...
4. **Does a doc comment on a public symbol state WHAT instead of WHY?**
   ...
   first lines of the body — a comment whose content is recoverable from the name or
...
5. **Is new public API naked, or a feature-sized change undocumented at rung 2?**
   ...
   against `<docroot>` contents.
   Violation: a new public type or package with no doc comment; or a
