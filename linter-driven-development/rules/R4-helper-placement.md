...
Build each search over the language's source files (`detected-language source`); "test files"
means the files the repository's test runner picks up (`*<test-file suffix>`, a `tests/`
