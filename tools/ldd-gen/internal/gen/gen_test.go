...
const handbookTemplate = "<!-- Generated by tools/ldd-gen -->\n# {{.Lang}}\n{{include \"handbook/notes.md\"}}\n"

func handbookRepo(t *testing.T) string {
	t.Helper()
	root := miniRepo(t)
	write(t, root, "lang/p/profile.yaml", profileYAML+"handbook: coding-rules/p.md\n")
	write(t, root, "lang/p/handbook/notes.md", "notes\n")
	write(t, root, "core/handbook/coding-rules.md", handbookTemplate)
	return root
}

func TestHandbook_GenerateThenCheck(t *testing.T) {
	t.Parallel()
	root := handbookRepo(t)
	repo, err := gen.Open(root)
	require.NoError(t, err)
	...
	n, err := repo.Check(&out)
	require.NoError(t, err)
	assert.Equal(t, 4, n, "three plugin files and the handbook are missing")
	assert.Contains(t, out.String(), "missing       coding-rules/p.md")

	require.NoError(t, repo.Generate("p"))
	got, err := os.ReadFile(filepath.Join(root, "coding-rules/p.md"))
	require.NoError(t, err)
	assert.Equal(t, "<!-- Generated by tools/ldd-gen -->\n# Lang\nnotes\n", string(got))
	assert.NoFileExists(t, filepath.Join(root, "out-plugin/handbook/coding-rules.md"), "the template is not a plugin file")
	...
	write(t, root, "lang/p/handbook/notes.md", "edited\n")
	out.Reset()
	n, err = repo.Check(&out)
	require.NoError(t, err)
	assert.Equal(t, 1, n)
...
func TestHandbook_RefusesToOverwriteAHandWrittenFile(t *testing.T) {
	t.Parallel()
	root := handbookRepo(t)
	write(t, root, "coding-rules/p.md", "# my own notes\n")
	repo, err := gen.Open(root)
	require.NoError(t, err)
	err = repo.Generate("p")
	require.Error(t, err)
	assert.Contains(t, err.Error(), "not a generated handbook")
	got, err := os.ReadFile(filepath.Join(root, "coding-rules/p.md"))
	require.NoError(t, err)
