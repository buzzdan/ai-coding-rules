...
1. **Absence.** `None` is a declared absence, never an undeclared failure. A
   ...
   it, checked by ty — `dict.get` beside `dict[k]` is the model. The findings are
   `None` returned where the signature promises `X` (R1 Q4, with
   `# ty: ignore[invalid-return-type]` as the silenced form), `None` standing in for a
...
sections are written in Python and may argue differently where Python differs: ty
