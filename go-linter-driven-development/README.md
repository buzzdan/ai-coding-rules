...
A request that delivers no new behavior ("fix the design of", "remove the global") skips Phase 2 entirely: it runs as Phase 1.5 in its own right, every move applied by @refactoring — which owns the moves, the five-step stopping criteria and the commit of each green step — then Phases 3, 4 and 5 as for any slice. The workflow never reshapes code by hand.
