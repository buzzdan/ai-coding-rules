...
|-------|-----------|-----------------|--------|
| [`rule-hunter`](agents/rule-hunter.md) | `@pre-commit-review` (one per rule family with hits, six at most, in parallel) | the family's `rules/R*.md` files with hits + the scope bundle | No (read-only) |
...
Every finding opens with its `file:line` anchor, answers the rule's falsifying question by number and in the question's own words (or carries a command's output), and names the move as the rule's Fix pattern spells it for HOW to fix; findings that share one shape may share one line that lists every anchor. A count of findings is never a finding: a whole-repository review with a hundred findings renders a hundred anchors.
