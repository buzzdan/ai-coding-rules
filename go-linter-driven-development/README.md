...
4. **Merged report**: surviving findings categorized as 🐛 Bugs / 🔴 Design Debt / 🟡 Readability Debt / 🟢 Polish. All advisory — the caller decides what to fix. Every finding is its own line: the `file:line` anchor, the rule and the falsifying question it answers (by number, in the question's own words), the move named as the rule's Fix pattern spells it, and effort — never a per-rule count, whatever the scope. Findings from *different* rules converging on one anchor (the same type, field, or function) are additionally reported as a 🔗 **CLUSTER**, one entry per converged anchor — each hunter is blind to the others, so independent convergence is evidence of a missing domain concept, and the cluster routes design-first (`@code-designing` scoped to the concept, then `@refactoring` implements) instead of being fixed member-by-member.
...
Every finding is one line that opens with its `file:line` anchor, answers the rule's falsifying question by number and in the question's own words (or carries a command's output), and names the move as the rule's Fix pattern spells it for HOW to fix. A count of findings is never a finding: a whole-repository review with a hundred findings renders a hundred lines.
