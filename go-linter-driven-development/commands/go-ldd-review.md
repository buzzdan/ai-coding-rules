...
2. **Run in read-only mode**:
   ...
   - Review: invoke `Skill(go-linter-driven-development:pre-commit-review)` in FULL mode over the resolved
     scope, passing it the file list. It orchestrates parallel `go-linter-driven-development:rule-hunter` agents + the
     `go-linter-driven-development:overabstraction-skeptic` and reports — it never edits, and it never widens the scope.
