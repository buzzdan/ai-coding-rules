...
repository that has never run go-ldd is a different job: expect the loop to run
long, and prefer `/go-ldd-analyze --all` first to see the size of it.
...
1. **An argument names it** (`$ARGUMENTS`). A file pattern or a directory fixes those
   ...
   repository, fixes every `*.go` file. The whole repository is never touched
   without being asked.
...
**Use the Skill tool** to invoke `Skill(go-linter-driven-development:linter-driven-development)` with the
resolved scope, telling it Phases 1–2 are skipped:

**Phase 3 — TESTS + LINT over the scope** (via the `lint-fixer` agent, Agent tool,
isolated context, spawned with the package list)
- Discover project test/lint commands (`task test` / `make test` / `go test ./...`;
  lint from Taskfile/Makefile or `golangci-lint run --fix`)
...
- @pre-commit-review orchestrates parallel `rule-hunter` agents + the
  `overabstraction-skeptic` against the diff; it reports, never edits, never widens
