...
- **Six rule-family hunters.** The pre-commit-review skill spawns one rule-hunter per
  rule family with pre-filter hits — types (R1, R2), dispatch and mutation (R11, R12),
  structure (R3, R4, R5), tests (R6, R7), state (R8, R10), documentation (R9, beside
  the comment critic) — six at most, no family over three rules, where it spawned one
  per rule, up to twelve. A hunter gets
  ...
  tokens, nothing else. `/go-ldd-review` no longer runs the tests or the linter: a
  review-only command reads code, it does not verify it; `/go-ldd-analyze` still runs
