...
const (
	...
	ExitBudget   = 2
	ExitUsage    = 3
	usage        = "usage: ldd-eval run [flags] <evals-dir>\n"
	flagsPreface = "flags:\n"
)
...
func Run(args []string, stdout, stderr io.Writer) int {
	...
	opts, err := parseRunFlags(args[1:], stdout, stderr)
	if err != nil {
		return ExitUsage
	}
...
func parseRunFlags(args []string, stdout, stderr io.Writer) (runner.Options, error) {
	fs := flag.NewFlagSet("ldd-eval run", flag.ContinueOnError)
	fs.SetOutput(stderr)
	...
	fs.StringVar(&opts.PluginDir, "plugin-dir", "", "plugin root passed to claude --plugin-dir (default: parent of <evals-dir>)")
	fs.BoolVar(&opts.KeepTemp, "keep-temp", false, "keep the scaffold dirs (path recorded in result.json)")
	fs.StringVar(&opts.OutDir, "out", "", "output dir (default: <evals-dir>/results/<timestamp>)")
